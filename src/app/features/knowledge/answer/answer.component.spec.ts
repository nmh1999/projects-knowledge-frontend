...
describe('Basic summary answer', () => {
  for (const language of ['en', 'ar'] as const) {
    it(`shows only the scope notice and never unrelated generated content in ${language}`, async () => {
      ...
      const answer: KnowledgeAnswer = {
        inScope: false, project: 'Project', question: 'Tell me a joke', summary: 'UNRELATED CONTENT', confidence: 'high', enoughEvidence: true,
        businessFlow: ['UNRELATED CONTENT'], technicalFlow: [], apis: [], database: [], integrations: [], scheduledJobs: [],
        technicalDetails: [], sources: [], keyFindings: ['UNRELATED CONTENT'], roles: [], risks: [], followUpQuestions: [], workflowExample: 'UNRELATED CONTENT'
      };
      fixture.componentRef.setInput('answer', answer); fixture.detectChanges();
      ...
      fixture.componentRef.setInput('answer', { ...answer, inScope: undefined }); fixture.detectChanges();
      expect(host.textContent).not.toContain('UNRELATED CONTENT');
      expect(host.querySelector('.scope-notice')).not.toBeNull();
      fixture.componentRef.setInput('answer', { ...answer, inScope: true, enoughEvidence: false, confidence: 'low', summary: 'No project evidence found' });
  ...
  it('uses the shared summary card without sources or detailed sections', async () => {
    ...
    const fixture = TestBed.createComponent(AnswerComponent);
    const answer: KnowledgeAnswer = {
      inScope: true, project: 'Project', question: 'Which framework?', summary: 'Uses Angular.', confidence: 'high', enoughEvidence: true,
      businessFlow: [], technicalFlow: [], apis: [], database: [], integrations: [], scheduledJobs: [],
      technicalDetails: [], sources: [], keyFindings: [], roles: [], risks: [], followUpQuestions: []
    };
    ...
    for (const language of ['en', 'ar'] as const) for (const confidence of ['high', 'medium', 'low'] as const) {
      const labels = TestBed.inject(LanguageService);
      labels.current.set(language);
...
describe('Workflow answer', () => {
  for (const language of ['en', 'ar'] as const) {
    it(`renders roles, steps and a labelled example in ${language}, including section and full copy`, async () => {
      ...
      const labels = TestBed.inject(LanguageService);
      labels.current.set(language);
      const example = language === 'ar' ? 'افترض أن المراجع يراجع طلبًا مقدمًا.' : 'Imagine a reviewer reviewing a submitted request.';
      const answer: KnowledgeAnswer = {
        inScope: true, project: 'Project', question: 'Explain reviews', summary: 'Review process', confidence: 'high', enoughEvidence: true,
        businessFlow: ['The reviewer reviews the submitted request.'], technicalFlow: [], apis: [], database: [], integrations: [], scheduledJobs: [],
        technicalDetails: [], sources: [], keyFindings: [], roles: [{ role: 'REVIEWER', capability: 'Reviews', evidence: 'Review check' }],
        risks: [], followUpQuestions: [], workflowExample: example,
        workflowDiagram: { nodes: [
