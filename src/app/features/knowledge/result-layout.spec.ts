...
import { SourceReference } from '../../core/models/knowledge-answer.model';
  ...
  for (const theme of ['light', 'dark']) {
    for (const direction of ['ltr', 'rtl']) {
      for (const width of [390, 1100]) {
        it(`keeps sources and actions inside ${width}px in ${theme}/${direction}`, () => {
          ...
          const source: SourceReference = {
            repositoryId: 'repo', repositoryName: 'public-marts-backend',
            ...
            startLine: 10, endLine: 50, excerpt: ''
          };
      ...
      it(`wraps technical endpoints inside cards in ${theme}/${direction}`, () => {
        ...
        fixture.componentRef.setInput('nodes', Array.from({ length: 6 }, () => ({
          type: 'api', name: 'Repeated lookup',
