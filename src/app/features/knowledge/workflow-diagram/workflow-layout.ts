import { WorkflowDiagram, WorkflowEdge, WorkflowNode } from '../../../core/models/workflow-diagram.model';

export type WorkflowOrientation = 'vertical' | 'horizontal';
export interface Point { x: number; y: number; }
export interface PositionedNode extends WorkflowNode { x: number; y: number; rank: number; }
export interface PositionedEdge extends WorkflowEdge { path: string; points: Point[]; x: number; y: number; description: string; loop: boolean; }
export interface WorkflowLayout { nodes: PositionedNode[]; edges: PositionedEdge[]; width: number; height: number; }
export const NODE_WIDTH = 248;
export const NODE_HEIGHT = 132;
export interface WorkflowGeometry { nodeHeight: number; labelWidth: number; labelHeight: number; }

const edgeKey = (edge: WorkflowEdge) => JSON.stringify([edge.from, edge.to]);
...
function roundedPath(points: Point[]): string {
  ...
  for (let i = 1; i < clean.length - 1; i++) {
    const previous = clean[i - 1], point = clean[i], next = clean[i + 1];
...
export function layoutWorkflow(graph: WorkflowDiagram | undefined, rtl = false, geometry?: WorkflowGeometry, orientation: WorkflowOrientation = 'vertical'): WorkflowLayout {
  const empty: WorkflowLayout = { nodes: [], edges: [], width: 0, height: 0 };
  if (!graph || !Array.isArray(graph.nodes) || !graph.nodes.length || !Array.isArray(graph.edges) || graph.nodes.length > 10 || graph.edges.length > 16) return empty;
  const ids = new Set<string>();
  for (const node of graph.nodes) {
    if (!node || typeof node.id !== 'string' || !node.id.trim() || typeof node.title !== 'string' || !node.title.trim()
      || typeof node.actor !== 'string' || ids.has(node.id) || !['start', 'action', 'decision', 'end'].includes(node.type)) return empty;
    ids.add(node.id);
  }
  if (graph.edges.some(edge => !edge || !ids.has(edge.from) || !ids.has(edge.to) || typeof edge.label !== 'string')) return empty;

  const pairs = new Map<string, WorkflowEdge>();
  for (const edge of graph.edges) {
    const previous = pairs.get(edgeKey(edge));
    pairs.set(edgeKey(edge), previous ? { ...edge, label: [...new Set([previous.label, edge.label].filter(Boolean))].join(' / ') } : { ...edge });
  }
  ...
  const visited = new Set<string>(), active = new Set<string>(), back = new Set<string>();
  ...
  const roots = [...graph.nodes].sort((a, b) => {
    const priority = (node: WorkflowNode) => node.type === 'start' ? 0 : !incoming.has(node.id) ? 1 : 2;
    return priority(a) - priority(b);
  });
  roots.forEach(node => { if (!visited.has(node.id)) visit(node.id); });
  ...
  const groups = Array.from({ length: Math.max(...ranks.values()) + 1 }, (_, rank) => graph.nodes.filter(node => ranks.get(node.id) === rank));
  const positions = new Map<string, number>();
  const updatePositions = () => groups.forEach(group => group.forEach((node, index) => positions.set(node.id, index - (group.length - 1) / 2)));
  updatePositions();
  ...
  for (let pass = 0; pass < 4; pass++) {
    ...
    for (const layer of layers) {
      const score = (node: WorkflowNode) => {
        const neighbors = forward.filter(edge => reverse ? edge.from === node.id : edge.to === node.id)
          .map(edge => positions.get(reverse ? edge.to : edge.from)!);
        return neighbors.length ? neighbors.reduce((sum, value) => sum + value, 0) / neighbors.length : positions.get(node.id)!;
      };
  ...
  const labelWidth = geometry?.labelWidth ?? 150, labelHeight = geometry?.labelHeight ?? 28;
  const mainSize = vertical ? nodeHeight : NODE_WIDTH, crossSize = vertical ? NODE_WIDTH : nodeHeight;
  const labelMain = vertical ? labelHeight : labelWidth, labelCross = vertical ? labelWidth : labelHeight;
  const crossGap = 72, laneGap = labelCross + 28, padding = 40;
  ...
  const transitions = groups.map((_, rank) => normal.filter(edge => ranks.get(edge.from) === rank).sort((a, b) => {
  ...
  const width = vertical ? crossExtent : mainExtent, height = vertical ? mainExtent : crossExtent;
  ...
  const port = (edge: WorkflowEdge, from: boolean) => {
    const id = from ? edge.from : edge.to;
    const peers = edges.filter(other => (from ? other.from : other.to) === id)
      .sort((a, b) => centers.get(from ? a.to : a.from)! - centers.get(from ? b.to : b.from)! || edgeKey(a).localeCompare(edgeKey(b)));
    const index = peers.indexOf(edge);
    const spacing = Math.min(24, crossSize * .5 / Math.max(1, peers.length - 1));
    return centers.get(id)! + (index - (peers.length - 1) / 2) * spacing;
  };
  const routed = edges.map(edge => {
    const from = byId.get(edge.from)!, to = byId.get(edge.to)!;
    const startMain = mainStarts[from.rank] + mainSize + 2, endMain = mainStarts[to.rank] - 5;
    const startCross = port(edge, true), endCross = port(edge, false);
    ...
    if (to.rank === from.rank + 1) {
      ...
      const middle = mainStarts[from.rank] + mainSize + 16 + (lane + .5) * (labelMain + 12);
      route = [point(startMain, startCross), point(middle, startCross), point(middle, endCross), point(endMain, endCross)];
      label = point(middle, (startCross + endCross) / 2);
    } else {
      const lane = (loop ? loops : skips).indexOf(edge);
      const outside = loop ? crossStart - (lane + .5) * laneGap : crossStart + crossSpan + (lane + .5) * laneGap;
      const depart = startMain + 14, arrive = endMain - 14;
      route = [point(startMain, startCross), point(depart, startCross), point(depart, outside), point(arrive, outside), point(arrive, endCross), point(endMain, endCross)];
      label = point((depart + arrive) / 2, outside);
    }
    return { ...edge, path: roundedPath(route), points: route, x: label.x, y: label.y, loop,
