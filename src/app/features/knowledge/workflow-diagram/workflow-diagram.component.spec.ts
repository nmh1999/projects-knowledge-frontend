...
import { layoutWorkflow, NODE_HEIGHT, NODE_WIDTH } from './workflow-layout';
import { pngDimensions, serializeWorkflowSvg, workflowPng } from './workflow-export';
import { createWorkflowScene } from './workflow-scene';

const graph: WorkflowDiagram = {
  nodes: [
...
describe('Workflow graph layout', () => {
  const merged: WorkflowDiagram = {
    ...
    edges: [
      { from: 's', to: 'd', label: '' }, { from: 'd', to: 'fast', label: 'No' }, { from: 'd', to: 'slow', label: 'Yes' },
      { from: 'fast', to: 'merge', label: 'Ready' }, { from: 'slow', to: 'audit', label: '' },
      { from: 'audit', to: 'merge', label: 'Verified' }, { from: 'merge', to: 'end', label: '' },
      { from: 'audit', to: 'd', label: 'Review again' }, { from: 'd', to: 's', label: 'Return' }
    ]
  };
  ...
  it('keeps paths outside nodes and labels separated in both orientations and languages', () => {
    for (const orientation of ['vertical', 'horizontal'] as const) for (const rtl of [false, true]) {
      ...
      const labels = scene.edges.filter(edge => edge.label).map(edge => ({ x: edge.x - edge.labelWidth / 2, y: edge.y - edge.labelHeight / 2, w: edge.labelWidth, h: edge.labelHeight }));
      for (const [index, label] of labels.entries()) {
        expect(label.x).toBeGreaterThanOrEqual(0); expect(label.y).toBeGreaterThanOrEqual(0);
        expect(label.x + label.w).toBeLessThanOrEqual(scene.width); expect(label.y + label.h).toBeLessThanOrEqual(scene.height);
      ...
      for (const edge of scene.edges) for (let i = 1; i < edge.points.length; i++) {
        const a = edge.points[i - 1], b = edge.points[i];
        const route = { x: Math.min(a.x, b.x), y: Math.min(a.y, b.y), w: Math.abs(b.x - a.x), h: Math.abs(b.y - a.y) };
  ...
  it('gives each decision exit a distinct, stable color without changing transitions', () => {
    for (const dark of [false, true]) {
      ...
      for (const edge of scene.edges) {
        expect(mirrored.edges.find(other => other.from === edge.from && other.to === edge.to)!.color).toBe(edge.color);
      }
...
describe('Workflow diagram rendering', () => {
  ...
  it('opens at readable scale and switches orientation without changing business transitions', () => {
    ...
    fixture.componentRef.setInput('diagram', graph); fixture.detectChanges();
    const component = fixture.componentInstance;
    const host = fixture.nativeElement as HTMLElement;
    host.style.width = '700px';
    component.fit('inline', true); fixture.detectChanges();
    expect(component.orientation()).toBe('vertical');
    expect(component.inlineScale()).toBeGreaterThanOrEqual(.75);
    const original = component.scene().edges.map(edge => [edge.from, edge.to]);
    host.querySelectorAll<HTMLButtonElement>('.orientation-controls button')[1].click(); fixture.detectChanges();
    ...
    expect(host.querySelectorAll<HTMLButtonElement>('.orientation-controls button')[1].getAttribute('aria-pressed')).toBe('true');
    ...
    component.scene().edges.forEach((edge, index) => expect(edges[index].getAttribute('stroke-dasharray')).toBe(edge.loop ? '6 5' : null));
    component.fit('inline'); fixture.detectChanges();
    expect(component.inlineScale()).toBeLessThan(.75);
    host.querySelector<HTMLButtonElement>('.readable-button')!.click(); fixture.detectChanges();
    expect(component.inlineScale()).toBeGreaterThanOrEqual(.75);
  });

  for (const theme of ['light', 'dark'] as const) for (const language of ['en', 'ar'] as const) {
  ...
  it('renders untrusted labels as text and wraps long titles within node bounds', () => {
    const fixture = TestBed.createComponent(WorkflowDiagramComponent);
    fixture.componentRef.setInput('diagram', { nodes: [{ ...graph.nodes[0], title: '<img src=x onerror=alert(1)>' + 'LongRole'.repeat(40) }], edges: [] });
    fixture.detectChanges();
    const host = fixture.nativeElement as HTMLElement;
  ...
  it('keeps multiline Arabic and English roles and branch conditions inside their shapes', () => {
    for (const language of ['en', 'ar'] as const) {
      const fixture = TestBed.createComponent(WorkflowDiagramComponent);
      TestBed.inject(LanguageService).current.set(language);
      const actor = language === 'ar' ? 'المسؤول عن مراجعة الطلب والتحقق من المستندات المرفقة' : 'Senior reviewer responsible for verifying the attached supporting documents';
      const label = language === 'ar' ? 'عند اكتمال جميع البيانات المطلوبة والموافقة على المستندات' : 'When all required information is complete and the documents are approved';
      fixture.componentRef.setInput('diagram', { ...graph,
      ...
      for (const group of host.querySelectorAll<SVGGElement>('.diagram-node')) {
        ...
        expect(text.x).toBeGreaterThanOrEqual(chip.x); expect(text.x + text.width).toBeLessThanOrEqual(chip.x + chip.width);
        expect(text.y).toBeGreaterThanOrEqual(chip.y); expect(text.y + text.height).toBeLessThanOrEqual(chip.y + chip.height);
      ...
      for (const group of host.querySelectorAll<SVGGElement>('.edge-label')) {
        ...
        expect(text.x).toBeGreaterThanOrEqual(box.x); expect(text.x + text.width).toBeLessThanOrEqual(box.x + box.width);
        expect(text.y).toBeGreaterThanOrEqual(box.y); expect(text.y + text.height).toBeLessThanOrEqual(box.y + box.height);
      }
      fixture.destroy();
    }
  });
  ...
  it('opens a native modal, zooms, fits, closes with Escape and restores focus and scrolling', () => {
    ...
    expect([...dialog.querySelectorAll('.edge-path')].map(path => path.getAttribute('stroke')))
      .toEqual(fixture.componentInstance.scene().edges.map(edge => edge.color));
    ...
    fixture.componentInstance.setZoom('modal', 10); fixture.detectChanges();
    ...
    fixture.componentInstance.setZoom('modal', 0); fixture.detectChanges();
    expect(fixture.componentInstance.modalScale()).toBe(.1);
    dialog.querySelector<HTMLButtonElement>('.fit-button')!.click(); fixture.detectChanges();
    ...
    dialog.querySelector('button')!.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true })); fixture.detectChanges();
    ...
    expect(dialog.open).toBeFalse();
    trigger.click(); fixture.detectChanges(); fixture.destroy();
    expect(document.body.style.overflow).toBe(overflow);
  });
  ...
  it('pans a zoomed diagram with the pointer', () => {
    const fixture = TestBed.createComponent(WorkflowDiagramComponent);
    fixture.componentRef.setInput('diagram', graph); fixture.detectChanges();
    fixture.componentInstance.setZoom('inline', 3); fixture.detectChanges();
    const viewport = (fixture.nativeElement as HTMLElement).querySelector<HTMLElement>('.diagram-scroll')!;
    spyOn(viewport, 'setPointerCapture'); spyOn(viewport, 'hasPointerCapture').and.returnValue(true);
  ...
  it('exports the entire Arabic SVG and a decodable PNG at any zoom', async () => {
    ...
    fixture.componentRef.setInput('diagram', { ...graph, nodes: graph.nodes.map(node => ({ ...node, title: 'مراجعة الطلب', actor: 'مقدم الطلب' })) });
    fixture.detectChanges();
    fixture.componentInstance.setZoom('inline', .2); fixture.detectChanges();
    ...
    expect(bitmap.width).toBe(size.width); expect(bitmap.height).toBe(size.height);
    bitmap.close();
