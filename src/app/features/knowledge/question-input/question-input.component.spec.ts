...
describe('Question search modes', () => {
  ...
  it('keeps history collapsed until opened and restores text and mode without sending', () => {
    ...
    fixture.componentRef.setInput('history', [entry]); fixture.detectChanges();
  ...
  it('disables history actions while loading and leaves the current draft untouched', () => {
    ...
    fixture.componentRef.setInput('history', [entry]); fixture.componentRef.setInput('loading', true);
    fixture.componentInstance.question.setValue('Current draft'); fixture.detectChanges();
    const cleared = spyOn(fixture.componentInstance.historyCleared, 'emit');
    const changed = spyOn(fixture.componentInstance.modeChanged, 'emit');
    for (const button of fixture.nativeElement.querySelectorAll('.question-history button') as NodeListOf<HTMLButtonElement>) {
    ...
    expect(cleared).not.toHaveBeenCalled(); expect(changed).not.toHaveBeenCalled();
  });

  it('renders safe, localized history with long questions in a narrow panel', () => {
    const host = fixture.nativeElement as HTMLElement;
    host.style.display = 'block'; host.style.width = '320px';
    fixture.componentRef.setInput('history', Array.from({ length: 5 }, (_, index) => ({ question: `${index} <img src=x onerror=alert(1)> ` + 'مرحباLongQuestion'.repeat(30), mode: 'workflow' })));
    fixture.detectChanges(); host.querySelector<HTMLDetailsElement>('.question-history')!.open = true;
    for (const language of ['en', 'ar'] as const) {
      fixture.componentInstance.language.current.set(language); host.dir = language === 'ar' ? 'rtl' : 'ltr'; fixture.detectChanges();
      expect(host.querySelectorAll('.history-question').length).toBe(5);
      expect(host.querySelector('.question-history summary')?.textContent).toContain(fixture.componentInstance.language.t('recentQuestions'));
      expect(host.querySelector('.history-mode')?.textContent).toBe(fixture.componentInstance.language.t('workflowMode'));
    ...
    fixture.componentRef.setInput('history', []); fixture.componentRef.setInput('historyPersistent', false); fixture.detectChanges();
    expect(host.querySelector('.history-empty')?.textContent).toBe(fixture.componentInstance.language.t('noRecentQuestions'));
    expect(host.querySelector('.history-warning')?.textContent).toBe(fixture.componentInstance.language.t('questionHistoryTemporary'));
  });
  ...
  it('selects Workflow without submitting and displays its own hint', () => {
    ...
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.mode-hint').textContent).toContain(fixture.componentInstance.language.t('workflowHint'));
  ...
  it('localizes each mode and includes the selected project in the restored heading', () => {
    ...
    for (const language of ['ar', 'en'] as const) {
      ...
      expect(host.querySelector('h1')?.textContent).toBe(language === 'ar' ? 'اسأل عن Example API' : 'Ask anything about Example API');
  ...
  it('does not submit whitespace, composing text or a held Enter key', () => {
    ...
    fixture.nativeElement.querySelector('textarea').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', isComposing: true, bubbles: true }));
    fixture.nativeElement.querySelector('textarea').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', repeat: true, bubbles: true }));
    expect(asked).not.toHaveBeenCalled();
  });
  ...
  it('places the format above the question and the send button below without horizontal overflow', () => {
    ...
    for (const language of ['en', 'ar'] as const) {
      ...
      for (const width of [960, 320]) {
        ...
        expect(choices.every(choice => choice.getBoundingClientRect().top === choices[0].getBoundingClientRect().top)).toBeTrue();
