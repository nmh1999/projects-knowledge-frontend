...
export interface QuestionHistoryEntry { question: string; mode: SearchMode; }
...
export class QuestionHistoryService {
  ...
  forProject(projectId: string): readonly QuestionHistoryEntry[] { return this.entries().get(projectId) ?? []; }

  remember(projectId: string, question: string, mode: SearchMode): void {
    const text = question.trim();
    if (!projectId || !text || !validMode(mode)) return;
    const next = new Map(this.entries());
    next.set(projectId, [{ question: text, mode }, ...this.forProject(projectId).filter(entry => entry.question !== text)].slice(0, HISTORY_LIMIT));
    this.save(next);
  }
  ...
  private save(entries: Map<string, QuestionHistoryEntry[]>): void {
    ...
    } catch { this.persistent.set(false); } // Keep this session usable if storage is blocked or full.
  }
  ...
  private read(): Map<string, QuestionHistoryEntry[]> {
    ...
    catch { this.persistent.set(false); return result; }
    try {
      ...
      for (const group of parsed) {
        ...
        for (const value of group[1]) {
          if (!value || typeof value.question !== 'string' || !value.question.trim() || !validMode(value.mode)) continue;
    ...
    } catch { /* Ignore malformed local data without blocking questions. */ }
    return result;
  }
}
