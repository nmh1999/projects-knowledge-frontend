...
describe('Question history', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(Storage.prototype, 'setItem').and.callFake((_key, value) => { saved = value; });
    spyOn(Storage.prototype, 'removeItem').and.callFake(() => { saved = null; });
  });
  ...
  it('keeps only the last five unique, trimmed questions per scope, newest first', () => {
    ...
    expect(history.forProject('one').map(entry => entry.question)).toEqual(['Question 6', 'Question 5', 'Question 4', 'Question 3', 'Question 2']);
    ...
    history.remember('one', '   ', 'basic'); history.remember('', 'No selection', 'basic');
  ...
  it('survives reloads and clears only the selected scope', () => {
    const history = new QuestionHistoryService();
    history.remember('one', 'First', 'workflow'); history.remember('__proto__', 'Second', 'advanced');
  ...
  it('validates restored data and limits oversized histories without changing question text', () => {
    ...
    saved = JSON.stringify([null, ['bad', {}], [42, []], ['one', [null, {}, { question: ' ', mode: 'basic' }, { question: 'Ignore', mode: 'unknown' },
      { question: text, mode: 'workflow' }, { question: text, mode: 'basic' },
    ...
    saved = '{broken'; expect(new QuestionHistoryService().forProject('one')).toEqual([]);
    saved = '{}'; expect(new QuestionHistoryService().forProject('one')).toEqual([]);
  });
