...
export class ThemeService {
  ...
  toggle(): void {
    ...
    this.apply(theme);
    try { localStorage.setItem('projects-knowledge-theme', theme); } catch { /* storage is optional */ }
  }

  private savedTheme(): AppTheme {
    try { return localStorage.getItem('projects-knowledge-theme') === 'dark' ? 'dark' : 'light'; }
    catch { return 'light'; }
  }

  private apply(theme: AppTheme): void { document.documentElement.dataset['theme'] = theme; }
}
