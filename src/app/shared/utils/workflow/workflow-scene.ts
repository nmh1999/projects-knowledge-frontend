...
import {
  ...
  PositionedNode,
  WorkflowOrientation
} from '@shared/utils/workflow/workflow-layout';
...
export interface SceneNode extends PositionedNode {
  heading: string;
  titleLines: string[];
  actorLines: string[];
  actorY: number;
...
export interface SceneEdge extends PositionedEdge {
  lines: string[];
  labelWidth: number;
  labelHeight: number;
  color: string;
...
export function createWorkflowScene(
  ...
  const nodeHeight = Math.max(
    NODE_HEIGHT,
    ...[...text.values()].map((value) => 65 + value.titleLines.length * 21 + value.actorLines.length * 17)
  );
  ...
  const branchKeys = layout.edges
    .filter((edge) => decisions.has(edge.from))
    .map(edgeKey)
    .sort();
  ...
  return {
    ...
    rtl,
    orientation,
    title: translate('workflowDiagram'),
    palette: dark
      ? {
          background: '#0b1e2e',
          surface: '#102638',
    ...
    nodes: layout.nodes.map((node) => {
      ...
      return {
        ...node,
        ...content,
        actorHeight: content.actorLines.length * 17 + 10,
        actorY: nodeHeight - content.actorLines.length * 17 - 24,
        heading: translate('diagramType_' + node.type)
      };
    }),
    edges: layout.edges.map((edge) => {
      ...
      return {
        ...
        lines,
        labelWidth: 150,
        labelHeight: lines.length * 17 + 14,
