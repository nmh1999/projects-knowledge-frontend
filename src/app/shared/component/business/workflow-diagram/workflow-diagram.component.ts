...
import {WorkflowCanvasComponent} from '@shared/component/business/workflow-diagram/workflow-canvas.component';
import {createWorkflowScene} from '@shared/utils/workflow/workflow-scene';
import {WorkflowOrientation} from '@shared/utils/workflow/workflow-layout';
import {downloadWorkflow, serializeWorkflowSvg, workflowPng} from '@shared/utils/workflow/workflow-export';
...
@Component({
  ...
  standalone: true,
  imports: [NgTemplateOutlet, WorkflowCanvasComponent],
...
export class WorkflowDiagramComponent implements OnDestroy {
  ...
  readonly diagram = input<DtoWorkflowDiagram>();
  readonly orientation = signal<WorkflowOrientation>('vertical');
  readonly scene = computed(() =>
    createWorkflowScene(
      this.diagram(),
      this.language.isArabic(),
      this.theme.isDark(),
      (key) => this.language.t(key),
      this.orientation()
    )
  );
  ...
  @ViewChild('inlineViewport') set inlineViewport(value: ElementRef<HTMLElement> | undefined) {
    this.observe('inline', value);
  }
  @ViewChild('modalViewport') set modalViewport(value: ElementRef<HTMLElement> | undefined) {
    this.observe('modal', value);
  }
  ...
  constructor() {
    ...
    effect(() => {
      ...
      untracked(() =>
        this.nextFrame(() => {
          if (this.autoFit.inline) this.fit('inline', this.autoFit.inline === 'readable');
          if (this.modalOpen() && this.autoFit.modal) this.fit('modal', this.autoFit.modal === 'readable');
        })
      );
    });
  }
  ...
  changeOrientation(value: WorkflowOrientation): void {
    ...
    this.autoFit.inline = 'readable';
    this.autoFit.modal = 'readable';
    this.orientation.set(value);
  }

  scale(view: DiagramView): number {
    return view === 'inline' ? this.inlineScale() : this.modalScale();
  }
  percent(view: DiagramView): number {
    return Math.round(this.scale(view) * 100);
  }
  zoom(view: DiagramView, delta: number): void {
    this.setZoom(view, this.scale(view) + delta);
  }
  setZoom(view: DiagramView, scale: number): void {
    ...
    this.nextFrame(() => {
      if (viewport) {
        viewport.scrollLeft = (x * next) / previous - viewport.clientWidth / 2;
        viewport.scrollTop = (y * next) / previous - viewport.clientHeight / 2;
      }
    });
  }
  ...
  fit(view: DiagramView, readable = false): void {
    ...
    const scale = readable
      ? Math.max(0.75, Math.min(1, (viewport.clientWidth - 48) / graph.width))
      : Math.max(
          0.1,
          Math.min(1, (viewport.clientWidth - 48) / graph.width, (viewport.clientHeight - 48) / graph.height)
        );
  ...
  wheel(view: DiagramView, event: WheelEvent): void {
    if (event.ctrlKey || event.metaKey) {
      event.preventDefault();
      this.zoom(view, event.deltaY < 0 ? 0.1 : -0.1);
    }
  }
  ...
  private observe(view: DiagramView, reference: ElementRef<HTMLElement> | undefined): void {
    ...
    if (!reference) return;
    this.observers[view] = new ResizeObserver(() =>
      this.nextFrame(() => {
        if (this.autoFit[view]) this.fit(view, this.autoFit[view] === 'readable');
      })
    );
